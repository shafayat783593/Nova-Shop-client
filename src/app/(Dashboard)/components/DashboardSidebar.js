...
// import Link from 'next/link';
// import { usePathname } from 'next/navigation';
// import { LogOut, ChevronLeft, ChevronRight, LayoutDashboard, Store } from 'lucide-react';
// import { DASHBOARD_MENU, normalizeRole } from '@/config/dashboardConfig';
// import { useAuth } from '@/app/context/AuthContext';
...
//     if (loading) {
//         return (
//             <aside className="fixed lg:sticky top-0 left-0 z-50 h-screen w-[85px] bg-card border-r border-accent-10 flex items-center justify-center">
...
//     // ✅ Logic: ভেন্ডর এপ্রুভ না হলে শুধু নির্দিষ্ট মেনু দেখাবে
//     let menuItems = DASHBOARD_MENU[role] || [];

//     if (role === 'vendor') {
//         const isApproved = user?.shop?.status === 'approved';
//         if (!isApproved) {
//             menuItems = [
//                 { name: "Dashboard", path: "/dashboard/vendor", icon: LayoutDashboard },
//                 { name: "Shop Setup", path: "/vendor/shop-info", icon: Store },
//             ];
//         }
//     }
...
//             <aside
//                 style={{ width: sidebarWidth }}
//                 className={`fixed lg:sticky top-0 left-0 z-50 h-screen bg-card border-r border-accent-10 transition-all duration-300 ease-in-out shadow-sm flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
//             >
//                 <div className="flex flex-col h-full overflow-hidden">
//                     {/* Logo + Toggle */}
//                     <div className="p-6 flex items-center justify-between min-h-[90px]">
//                         {isOpen ? (
//                             <div className="text-2xl font-display font-black text-primary dark:text-accent truncate">TerraShop</div>
//                         ) : (
//                             <div className="text-2xl font-black text-primary mx-auto">T.</div>
//                         )}
//                         <button
...
//                                     <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${role === 'admin' ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'}`}>
//                                         {role} {user?.shop?.status !== 'approved' && role === 'vendor' ? '(Pending)' : ''}
//                                     </span>
//                                 </div>
//                             </div>
//                         </div>
...
//                                     href={item.path}
//                                     onClick={() => window.innerWidth < 1024 && setIsSidebarOpen(false)}
//                                     className={`relative flex items-center gap-3 rounded-2xl font-bold transition-all duration-200 group ${isOpen ? 'px-4 py-3' : 'px-3 py-3 justify-center'} ${isActive ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-body hover:bg-primary/5 hover:text-primary'}`}
//                                 >
//                                     <Icon size={22} className="shrink-0" />
//                                     {isOpen && <span className="truncate">{item.name}</span>}
//                                     {!isOpen && (
//                                         <span className="absolute left-[70px] bg-heading text-white px-3 py-1.5 rounded-lg text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
//                                             {item.name}
//                                         </span>
//                                     )}
//                                 </Link>
