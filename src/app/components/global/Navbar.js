        ...
        {items.map((item) => (
          <Link key={item.name} href={item.href}
            ...
            <span className="text-xl w-9 h-9 flex items-center justify-center
                            bg-accent/20 rounded-xl group-hover:bg-accent/35
...
// ── Profile Dropdown ──────────────────────────────────────────────────
function ProfileDropdown({ show, onClose, user, profileData, badgeCounts, onLogout, loading }) {
  const initials = getInitials(profileData?.name || user?.name || user?.email);
  const displayName = profileData?.name || user?.name || user?.email || "User";
  const email = profileData?.email || user?.email || "";
  const avatar = profileData?.avatar || user?.avatar || null;
    ...
    <div className={`absolute top-full right-0 mt-4 w-72 z-50
      ...
      <div className="bg-card border border-accent/20 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div className="relative flex-shrink-0">
              {loading ? (
                ...
                <img src={avatar} alt={displayName}
                  className="w-[52px] h-[52px] rounded-2xl object-cover shadow-lg" />
              ) : (
                <div className="w-[52px] h-[52px] rounded-2xl bg-accent
                                    flex items-center justify-center text-lg font-black text-primary shadow-lg"
            ...
            <div className="min-w-0 flex-1">
              {loading ? (
                ...
                <>
                  <p className="font-black text-[15px] text-accent m-0 truncate"
                    style={{ fontFamily: "'Syne', sans-serif" }}>
                    {displayName}
                  </p>
        ...
        <div className="grid grid-cols-3 border-b border-accent/10">
          {[
            [loading ? "—" : (badgeCounts?.orders ?? profileData?.totalOrders ?? 0), "Orders"],
            [loading ? "—" : (profileData?.reviews ?? 0), "Reviews"],
            [loading ? "—" : (profileData?.points ?? 0), "Points"],
          ].map(([v, l], i) => (
            <div key={l}
              className={`text-center py-2.5 cursor-pointer
            ...
            return (
              <Link key={item.label} href={item.href} onClick={onClose}
                className="flex items-center justify-between
                                    px-3 py-2.5 rounded-xl hover:bg-accent/15
        ...
        <div className="p-2 pt-0 border-t border-accent/10">
          <button onClick={onLogout}
            className="w-full flex items-center justify-center gap-2
                            py-2.5 rounded-xl bg-danger/10 text-danger
                            text-sm font-bold hover:bg-danger/20
                            transition-colors border-none cursor-pointer nb-font">
            <LogOut size={15} />
            Sign Out
          </button>
        </div>
      ...
      <div className="bg-card border border-accent/20 rounded-2xl shadow-2xl overflow-hidden p-3 space-y-2">
        ...
        <Link href="/login"
          className="flex items-center justify-center w-full py-2.5 rounded-xl
                        bg-primary text-accent text-sm font-bold
...
export default function Navbar() {
  ...
  const [promoText, setPromoText] = useState("Loading offers...");

  // ── Real data from API ──
  const [profileData, setProfileData] = useState(null);
  const [badgeCounts, setBadgeCounts] = useState({ orders: 0, wishlist: 0, cart: 0 });
  ...
  useEffect(() => {
    const fetchPromo = async () => {
      try {
        ...
        if (res.data?.text) {
          // ডাবল করে দেওয়া যাতে এনিমেশন স্মুথ থাকে
          setPromoText(`${res.data.text} · ${res.data.text}`);
        }
      } catch (err) {
  ...
  // ── Fetch profile + badge counts when authenticated ──
  useEffect(() => {
    if (!isAuth || !user) {
      setProfileData(null);
      setBadgeCounts({ orders: 0, wishlist: 0, cart: 0 });
      return;
    }
  ...
  return (
    <>
      <style>{`
                ...
                .nb-spin { animation: spin 1s linear infinite; }
            `}</style>
      ...
      {/* <div className="bg-primary overflow-hidden h-8 flex items-center nb-font">
        <p className="nb-ticker text-accent text-xs font-bold tracking-wide">
          ...
          Code <strong>GREEN15</strong> = 15% off &nbsp;·&nbsp;
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            <Link href="/cart" aria-label="Cart"
              className="hidden sm:flex items-center gap-2
                                bg-primary hover:bg-secondary text-accent
                                h-[38px] px-4 rounded-xl text-sm font-bold
            ...
            <div className="hidden sm:block relative" ref={profileRef}>
              {authLoading ? (
                ...
                // Authenticated — show avatar button
                <>
                  <button
                    ...
                    <div className={`w-7 h-7 rounded-lg overflow-hidden flex-shrink-0
                                            ...
                                            ${profileOpen ? "ring-2 ring-accent ring-offset-2 ring-offset-card" : ""}`}>
                      {avatar ? (
                        <img src={avatar} alt={displayName}
                          className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full bg-primary flex items-center justify-center
                                                    text-accent text-xs font-black nb-font-display">
                          {initials}
                        </div>
                      )}
                    </div>
                    <span className="text-sm font-semibold text-heading hidden lg:block max-w-[80px] truncate">
                      {profileData?.name?.split(" ")[0] || user?.name?.split(" ")[0] || "Account"}
                    </span>
                    <ChevronDown size={13}
                      className={`text-body transition-transform duration-200
            ...
            <button
              ...
              className="lg:hidden w-[38px] h-[38px] rounded-xl
                                bg-accent/20 text-heading flex items-center justify-center
                                hover:bg-accent/30 transition-all border-none cursor-pointer ml-0.5">
              <Menu size={20} />
            </button>
          </div>
        </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-bg border border-accent/20 text-heading
                                    placeholder:text-body text-sm
                                    pl-10 pr-10 py-2.5 rounded-xl outline-none
                                    focus:border-accent transition-colors nb-font"
              />
              <button
                onClick={() => { setSearchOpen(false); setSearchQuery(""); }}
                className="absolute right-3 top-1/2 -translate-y-1/2
                                    text-body hover:text-heading transition-colors
      ...
      <aside className={`nb-drawer fixed top-0 left-0 h-full w-[285px] z-[60]
        ...
        {/* Drawer Profile Card — conditional on auth */}
        {authLoading ? (
          ...
          <div className="bg-primary px-4 py-4 flex-shrink-0 relative overflow-hidden">
            ...
            <div className="relative flex items-center gap-3">
              <div className="relative flex-shrink-0">
                {avatar ? (
                  <img src={avatar} alt={displayName}
                    className="w-12 h-12 rounded-[13px] object-cover shadow-lg" />
                ) : (
                  <div className="w-12 h-12 rounded-[13px] bg-accent flex items-center
                                        justify-center text-lg font-black text-primary shadow-lg nb-font-display">
                    {initials}
                  </div>
                )}
          ...
          {NAV_LINKS.map((link) => (
            <div key={link.label}>
              <Link href={link.href}
                ...
                className="nb-font flex items-center justify-between
                                    px-3 py-2.5 rounded-xl text-sm font-semibold
                                    text-body hover:text-heading hover:bg-accent/15
        ...
        <div className="flex-shrink-0 border-t border-accent/10 p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            {mounted && (
              <button onClick={toggleTheme}
                className="flex items-center justify-center gap-1.5
                                    py-2.5 rounded-xl bg-accent/15 text-heading
                                    text-xs font-bold hover:bg-accent/25
                                    transition-colors border-none cursor-pointer nb-font">
            ...
            <Link href="/cart" onClick={() => setDrawerOpen(false)}
              className="flex items-center justify-center gap-1.5
                                py-2.5 rounded-xl bg-primary text-accent
